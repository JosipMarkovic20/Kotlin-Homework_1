...
fun main() {
    ...
    val values = data.values.flatMap { it }
    val filteredValues = values.filter{it>0}
    val avgAge = filteredValues.sum()/filteredValues.size
    println(avgAge.toDouble())
    data.forEach { (key, value) -> if (value.contains(-1))println(key) }
    val faultyEntries = data.count{it.value.contains(-1)}
    println(faultyEntries)
